            ...
            for apt in valid_apartments[floor]:
                ...
                if not occupied:
                    ...
                    for p in people_list:
                        ...
                        print(f"{p.street_address} Section: {p.sections}, Floor:  {p.floor}, Apartment number: {p.apartment_number},  Tenant: {p.name}")
                    return False
    return True

def get_every_apartment(people_list):
    valid_apartments = {1: (1, 2, 3), 2: (4, 5, 6), 3: (7, 8, 9)}

    for section in ["A", "B"]:
        for floor in (1, 2, 3):
            for apt_num in valid_apartments[floor]:
                occupant = next(
                    (p for p in people_list
                     if p.sections == section and
                        p.floor == floor and
                        p.apartment_number == apt_num),
                    None
                )
                if occupant:
                    print(f"Section {section}, Floor {floor}, Apt {apt_num}: Tenant: {occupant.name}")
                else:
                    print(f"Section {section}, Floor {floor}, Apt {apt_num}: no tenants")